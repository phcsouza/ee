...
def save_data():
    ...
    # Abre o diálogo de salvamento, mudando a extensão padrão para .txt ou .dat
    # É mais prático salvar como .txt e definir o delimitador como tabulação.
    name = filedialog.asksaveasfile(mode='w', defaultextension=".txt") 
    
    if name: 
        # Fecha o arquivo tk (necessário para np.savetxt) e obtém o nome do caminho
        filepath = name.name 
        name.close() 

        # Usar np.savetxt é a chave. Ele lida com a formatação e a escrita.
        # - fmt='%.10f' garante 10 casas decimais.
        # - delimiter='\t' cria um TSV (tab separated values).
        np.savetxt(filepath, pp, fmt='%.10f', delimiter='\t')
